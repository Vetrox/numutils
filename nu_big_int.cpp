...
{
    auto bigger_sign = m_uint == other.m_uint ? false : (m_uint > other.m_uint ? m_is_negative : other.m_is_negative);
    auto add = m_uint + other.m_uint;
    return BigInt(std::move(add), bigger_sign);
}
BigInt BigInt::operator-(BigInt const& other) const
{
    bool bigger_sign = m_uint == other.m_uint ? false : (m_uint > other.m_uint ? m_is_negative : !other.m_is_negative);
